...
**Block bootstrap (fixed a real methodological limitation):** returns were originally drawn i.i.d. (one independent single-year pick at a time), which ignores real bull/bear market clustering entirely. Verified against this project's own data: real 30-year rolling CAGR windows in actual history never exceeded ~13.6% (min ~8.0%, median ~10.8%, across 69 overlapping windows in the 98-year dataset), but independent single-year resampling produced trials up to ~24% CAGR over 30 years — growth that's never actually happened. Fixed via `sampleBlockBootstrapReturns`: instead of picking each year independently, it repeatedly picks a random contiguous slice of `blockLengthYears` REAL consecutive historical years (in their real order) and concatenates blocks until the needed length is reached (final block truncated as needed). This preserves real momentum/clustering *within* each block while still varying *which* blocks (and in what order) get picked across trials — narrowed the same 30-year Monte Carlo max CAGR down to ~19-20% in testing, a large improvement though not a perfect match to the tighter real-history bound (500 trials isn't enough to rule out ever exceeding it, and blocks from different eras can still combine in ways no single real 30-year window did).

**Block Length — later made user-facing (revised from the original "internal only" decision):** initially shipped as an internal-only constant (5 years, not exposed). Once the user started directly comparing different assumptions (Historical Data Start Year), they asked to tune this too — reasonably, since 5 years felt short for capturing a full cycle. Rather than pick one fixed value on their behalf, added **Block Length (years)** as a real input (validated 1–20), defaulting to **7 years** (a middle ground between the original 5 and the user's suggested 10) so they can compare 7 vs. 10 vs. anything else directly instead of requesting a hardcoded change each time. Larger blocks preserve more of a real cycle intact but leave fewer distinct starting points (89 possible 10-year starts vs. 94 for 5-year, in the 98-year dataset) — a real tradeoff with no universally "correct" answer, which is exactly why it's user-adjustable rather than fixed.
...
4. In an **up** year, any outstanding loan is repaid from stock gains **before** the Cash Bucket is refilled (confirmed via explicit user choice) — the loan accrues interest every year it's outstanding, unlike the Cash Bucket, which just sits idle, so paying it down first is strictly cheaper. The repayment is grossed up for tax like any other withdrawal (see bugfix below) and can mean the Cash Bucket is only partially refilled (or not refilled at all) in a year where a large loan also needs repaying.
...
**"Legacy backstop" observation and follow-up (Summary metric, not an engine change):** because the loan mechanic only ever taps the cash value reactively (down years, capped, never in up years, and never once the modeled stock/cash portfolio itself hits zero — see 13.3's `runWithdrawalTrack` break condition), a trial can fully deplete its modeled portfolio while real, untapped cash value is still sitting in the policy. Rather than changing the simulation to spend that value down too (considered — would mean drawing against WL capacity regardless of return sign once stock/cash are exhausted, extending the depletion year itself — deferred as a bigger behavioral change than the user asked for), a **"Whole Life Value Remaining (Net of Loan)"** metric was added to the Summary (`DistributionSummary.tsx`): the median trial's cash value minus any outstanding loan, read directly off `medianTrialRows`' last row (which is always the depletion-year row, or the final row if the trial survived — no new calculation needed). Explicitly framed as a backstop/legacy figure outside the modeled portfolio, not counted toward the success rate. Only rendered when the buffer is actually in use (final row's cash value > 0), so it's invisible noise for the ~everyone-else not using it.

**Bugfix — loan repayment wasn't taxed (caught by the user, not by testing):** the original repayment code (`stockBalance -= loanRepayment`) pulled money straight out of the modeled portfolio to pay the insurer with no tax at all. That's inconsistent with everything else in this tool: the cash-bucket refill right next to it is correctly untaxed because it's a same-account reallocation (stock sub-account → cash sub-account, both still inside the modeled tax-deferred/Roth portfolio), but the whole life cash value is explicitly a **separate** asset (item 3 above) — repaying it sends money to an external party, which is a real distribution from the account, taxable exactly like the year's spend withdrawal. The bug silently made the loan buffer look better than it should: understated tax owed and overstated ending balances/success rate/legacy value in every trial where a loan actually got repaid. Fixed by grossing up the repayment through the same `solveGrossWithdrawal`/`computeCombinedTaxOwed` machinery as any other withdrawal — sharing whatever standard-deduction headroom the year's spend withdrawal hasn't already used (not a fresh deduction), respecting `rothPortfolioPct` (a Roth share still repays tax-free, correctly), and capped at available stock if the full grossed-up repayment isn't affordable (partial repayment, same as before, just tax-correct). The extra tax is added into that year's existing `taxOwed` figure rather than a new column — verified live: normal years run ~$21-45K in this tool's example scenario, years with a loan repayment now correctly spike as high as $142K.
...
18. ✅ Whole Life "legacy backstop": user observed some depleted trials still show real WL cash value/death benefit sitting unused, since the loan mechanic only taps it reactively and never once the modeled portfolio itself hits zero. Considered extending the simulation to spend it down too (bigger change, deferred) vs. just surfacing what's left — user chose the latter. Added a Summary-only metric (no engine change): net cash value (minus any loan) off the median trial's last row, framed explicitly as outside the modeled portfolio (13.13)
19. ✅ Whole Life loan repayment bugfix: caught by the user (not by testing) — repaying the loan pulled money out of the modeled portfolio into the insurer with no tax at all, unlike every other withdrawal. Fixed to gross up the repayment the same way, sharing that year's already-partially-used standard deduction rather than a fresh one, respecting Roth Percentage. Understated tax/overstated success rate and legacy value in every trial where a loan actually got repaid, until fixed (13.13)
